...
```{r}
#For Full Model
p2 <- ggplot(data=Bootstrap_Results_M2, aes(x=bootstrap_b0)) + geom_histogram() + geom_vline(xintercept=quantile(bootstrap_b0, c(.025, .975)), color="red")
p2
...
M_Full <- lm(price ~ bedrooms + room_type + overall_satisfaction, data=Airbnb)
anova(M_Red, M_Full)
```
...
Mb_Full <- lm(data=B_Data, SimPrice ~ bedrooms + room_type + overall_satisfaction)   #fit full model to simulated data
...
```{r}
p1 <- ggplot(data=Bootstrap_Results_f_stat, aes(x=bootstrap_F)) + geom_histogram() + geom_vline(xintercept = 38.914, color="red", linetype="dotted", size=2) 
p1

mean(bootstrap_F>34.276)
```
...
References: https://www.sciencedirect.com/topics/earth-and-planetary-sciences/bootstrapping#:~:text=Parametric%20bootstrapping%20works%20as%20follows,the%20most%20likely%20parameter%20estimates
