...
<<<<<<< HEAD
B_Data <- B_Data %>% mutate(SimPrice = b0 + (b1*B_Data$bedrooms) + b2 + b3 + rnorm(n= nrow(Airbnb), mean=0, sd=sigma))
Mb_Reduced <- lm(data=B_Data, SimPrice ~ bedrooms + room_type)   #fit reduced model to simulated data
Mb_Full <- lmer(data=B_Data, SimPrice ~ bedrooms + room_type + overall_satisfaction + (1|neighborhood)) #fit full model to simulated data
bootstrap_F[i] <- anova(Mb_Full, Mb_Reduced)$F[2]
=======
B_Data <- B_Data %>% mutate(SimPrice = b0 + b1*yearnew + rnorm(n= nrow(), mean=0, sd=sigma))
...
>>>>>>> c8c919fb9d13653b71346c7dea92d8b78c6ef4e2
}
Bootstrap_Results <- data.frame(bootstrap_F)
```
Confidence Intervals for Reduced and Full Models...
...
```{r}
p2 <- ggplot(data=Bootstrap_Results, aes(x=bootstrap_F)) + geom_histogram() + geom_vline(xintercept = 223.73, color="red", linetype="dotted", size=2) 
p2
```
